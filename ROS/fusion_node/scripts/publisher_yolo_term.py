...
"""
===========================================================================
Nombre del archivo: publisher_yolo_term.py

Descripción:
    Nodo de ROS que utiliza un modelo YOLOv8 para detectar objetos en imágenes
    térmicas comprimidas recibidas desde un tópico. Publica todas las detecciones
    (clase, confianza y bounding box) en un mensaje personalizado tipo DetectionArray.

Funcionalidades:
    1. Suscribe a imágenes térmicas comprimidas desde el tópico `/term_img_coincidence`.
    2. Decodifica la imagen usando OpenCV.
    3. Infiere para detectar clases con un modelo YOLOv8 entrenado previamente.
    4. Publica las detecciones en el tópico `/thermal_detections` como mensajes tipo DetectionArray.
...
class YOLOThermalPublisher:
    def __init__(self):
        ...
        model_path = os.path.expanduser("~/modelos/modelo_term.pt")
        self.model = YOLO(model_path)

        # Publicador de detecciones en el tópico especificado
        self.pub = rospy.Publisher("/thermal_detections", DetectionArray, queue_size=10)

        # Suscriptor al tópico de imagen térmica comprimida
        rospy.Subscriber("/term_img_coincidence", CompressedImage, self.image_callback)
